...
|---|---|---|---|
| **Forward secrecy / PCS** | `K` is static and the per-message subkey is `HKDF(K, feedId‖seq)` — deterministic, *not* a ratchet — so a compromised `K` exposes every message under it. Replace the derivation with a forward-secure chain (advance a chain key per entry; or a Double Ratchet for PCS too). | `lib/src/messaging/message_crypto.dart` (`messageSubkey`), `Contact` state in `lib/src/pairing/models.dart`, the seq handling in `messaging_service.dart` | **High** — the relay can *gap* (TTL-sweep an undelivered entry), so you need skipped-message-key handling, not just a linear chain. This is the headline open problem. |
...
| **Metadata resistance** | The relay sees sizes, cadence, IP, and the pairwise graph. Sizes/cadence are *designed* away but not built. | pad in `Crypto.seal` callers to size buckets; fixed-cadence polling in the loops (`connect_screen` `_poll`, `Conversation.sync`) | **Medium** for padding/cadence; **Hard** for IP/timing (needs a mixnet — out of scope for a free relay, but a fascinating fork). |
| **Safety-number enforcement** | The TOFU→verified upgrade exists (`Contact.safetyNumber`) but is shown on demand, never enforced — it relies on users choosing to check. | add `Contact.verified`, a pre-first-send nudge + QR compare; `lib/src/contacts/conversation_screen.dart` (`_showSafetyNumber`), `models.dart` | **Low–Medium** — high security ROI for little code. A great first issue. |
