...
- **Content keys — a per-feed hash ratchet.** Pairing's ECDH yields a conversation root `K`, which is immediately split into two per-feed **chain roots** (`HKDF(K, feedId)`, length-prefix-bound) — and `K` is then discarded, never persisted. Each entry is sealed with **XChaCha20-Poly1305** under `messageKey(CK_seq)`, and the chain steps one-way: `CK_{seq+1} = HKDF(CK_seq)`, the old key deleted as each side advances. The chain position *is* the entry seq, which already travels with every entry — so this is a ratchet with **zero synchronization cost**, which is what makes it the right amount of machinery for low-cadence missives (the original deferral reasoned the sync cost wasn't justified; a hash ratchet has none). A reader that finds a seq gap (an entry destroyed by the TTL sweep before collection) fast-forwards over it, consuming those keys forever. **No keys ever reach the relay in cleartext**, and nothing about the ratchet touches the relay at all.
- **Forward secrecy, and its honest limit.** The one-way chain means state held *now* cannot decrypt ciphertext captured *earlier*: delivered-and-destroyed missives stay destroyed even against a later device compromise. What the symmetric ratchet does **not** give is post-compromise security — today's chain state derives all future keys until the parties re-pair. Healing requires mixing fresh DH into the chain (the Double Ratchet); that remains the documented upgrade.
